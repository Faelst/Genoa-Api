...
const getContracts = async (req, res) => {
  ...
  if (req.body.type_text_id) {
    const { type_text_id } = req.body;
    filter.client_id = await db("clients")
      .select("id")
      .where({ type_text_id: type_text_id.replace(/[^\w\s]/gi, "") })
      .first();
    if (!filter.client_id)
  ...
  try {
    const contract = await db("contracts").where(filter).where({
      deleted: 0,
    });
  ...
  try {
    ...
    if (moment(inauguration_date).add(2, "Y").isSameOrAfter(moment()))
      return res.status(202).send({
        status: false,
        message:
          "inauguration_date is minor than 2 year, invalid contract term",
      });
...
const deleteContract = async (req, res) => {
  ...
  try {
    const [contract] = await db("contracts").where({
      id,
    });
...
const alterContract = async (req, res) => {
  ...
  try {
    ...
    // --------------- CASO UTILIZE MAIS QUE O ID DO CONTRATO PARA PODER INDENTIFICAR O CONTRATO A SER ALTERADO.
    // filter.id
    //   ? (filter = { "contracts.id": filter.id })
    //   : (filter = {
    //       "clients.type_text_id": filter.type_text_id.replace(/[^\w\s]/gi, ""),
    //     });

    const [contract] = await db("contracts").where({ id: filter.id });
    ...
    if (data.user_id) {
      const [user_id] = await db("users").where({
        deleted: 0,
        id: data.user_id,
      });
    ...
    if (data.client_id || data.type_text_id) {
      ...
      data.client_id
        ? (filterContract = { id: data.client_id })
        : (filterContract = {
            type_text_id: data.type_text_id.replace(/[^\w\s]/gi, ""),
          });

      const [client] = await db("clients")
        .where(filterContract)
        .where({ deleted: 0 });
    ...
    if (data.beginning_date) {
      data.beginning_date = moment(data.beginning_date, "DD/MM/YYYY").format(
        "YYYY-MM-DD"
      );
      if (
        moment(data.beginning_date).isAfter(contract.final_date) ||
        moment(data.beginning_date)
          .add(1, "Y")
          .isSameOrAfter(contract.final_date)
      )
        return res.status(202).send({
          status: false,
          message: "the dates of contract not correct, invalid contract term",
    ...
    if (data.final_date) {
      data.final_date = moment(data.final_date, "DD/MM/YYYY").format(
        "YYYY-MM-DD"
      );
      if (
        moment(contract.beginning_date).isAfter(data.final_date) ||
        moment(contract.beginning_date)
          .add(1, "Y")
          .isSameOrAfter(data.final_date)
      )
        return res.status(202).send({
          status: false,
          message: "the dates of contract not correct, invalid contract term",
